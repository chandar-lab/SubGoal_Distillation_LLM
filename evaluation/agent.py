...
from data.data_utils import clean, add_current_place, add_current_objects, sanitizeStr, formalize_action, recover_action
from data.data_utils import compose_instance_v4, compose_instance_v4_sg, compose_instance_v4_nohist, compose_instance_v4_sgaslabel,  \
    compose_instance_v4_sg_nohist, compose_instance_v4_nohist_2, get_real_task_id, compose_instance_v4_first_sgaslabel
from subgoals.sg_generating import sg_generator
from eval_utils import load_model, findValidActionNew, load_variation, get_model_output, load_model_sg, load_model_Firstsg
...
def eval(args, task_num, logger):
    ...
    print(args.keys())
    print("LM parameters : mode_sg (lm or rand), sg_(using sg or not), hist_ (using hist or not): ")
    ...
    if args["compose_mode_sg"] == "v4_for_sg":  ## we use this composition to create input instruction for the sg moel
    ...
    locations = ['kitchen', 'bathroom', 'workshop', 'art studio', 'greenhouse', 'outside']
    ...
    for variation in variations_intest[:10]:
        ...
        env.load(taskName, int(variation), args["simplification_str"], generateGoldPath=True)
        ...
        while not done:
            ...
            if step == 0:
                if args['sg_']:
                    ...
                    steps, subgoals = sg_generator(env.getGoldActionSequence(), env.taskdescription(), variation, env.getGoldActionSequence(), None, task_name_id)
                    modified_sg = []
                    ...
                    if args['mode_sg'] == 'rand' and step%10 == 0:
                        ...
                        input_first_sg, _ = compose_instance_v4_first_sgaslabel(mode=mode, step_id=step + 1, task_desc=task_description, inventory=info['inv'], look=info['look'], places=places, recent_scores=recent_scores, curr_subgoal=None)
                        first_subgoal = get_model_output(args, input_first_sg, tokenizer_Fsg, lm_model_Fsg, device, logger)[0].strip().strip('.')
                        first_subgoal = creat_semi_random_sg(first_subgoal, env.getPossibleObjects(), locations)
                    else:
                        input_first_sg, _ = compose_instance_v4_first_sgaslabel(mode=mode, step_id=step + 1, task_desc=task_description, inventory=info['inv'], look=info['look'], places=places, recent_scores=recent_scores, curr_subgoal=None)
                        first_subgoal = get_model_output(args, input_first_sg, tokenizer_Fsg, lm_model_Fsg, device, logger)[0].strip().strip('.')
                    ...
                    curr_subgoal = findValidActionNew(' ', env, info['look'], recent_actions, sbert_model, logger, 5, True, taskName, variation, curr_subgoal, task_description)
            ...
            if not args['sg_']:
                ...
                if step == 0:
                    ...
                    if args['mode_sg'] == 'rand' and step%10 == 0:
                        ...
                        print('curr_subgoal is', curr_subgoal, 'last action: ', recent_actions[-1], 'last obs: ', recent_obs[-1], 'last reward: ', recent_reward[-1])

                    elif args['mode_sg'] == 'semi-rand' and step%10 == 0 :
                        input_str_sg, _ = compose_instance_sg(mode=mode, step_id=step + 1, task_desc=task_description,
                                                returns_to_go=returns_to_go,
                                                curr_action=None, curr_obs=obs, inventory=info['inv'], look=info['look'],
                                                prev_action=prev_action, prev_obs=prev_obs, objects=objects, places=places,
                                                recent_actions=recent_actions, recent_obs=recent_obs,
                                                recent_scores=recent_scores, recent_reward=recent_reward,
                                                subgoals=modified_sg, curr_subgoal=None, last_sg=prev_subgoal)
                        
                        curr_subgoal = get_model_output(args, input_str_sg, tokenizer_sg, lm_model_sg, device, logger)[0].strip().strip('.')                     
                        curr_subgoal = creat_semi_random_sg(curr_subgoal, env.getPossibleObjects(), locations)
                        print('curr_subgoal is', curr_subgoal, 'last action: ', recent_actions[-1], 'last obs: ', recent_obs[-1], 'last reward: ', recent_reward[-1])
                ...
                if clean(curr_subgoal).strip().strip('.')  != clean(modified_sg[subgoal_index]).strip().strip('.'):
                    subgoal_index += 1
                    modified_sg.append(curr_subgoal)

                input_str, _ = compose_instance(mode=mode, step_id=step + 1, task_desc=task_description,
                                                returns_to_go=returns_to_go,
                                                curr_action=None, curr_obs=obs, inventory=info['inv'], look=info['look'],
                                                prev_action=prev_action, prev_obs=prev_obs, objects=objects, places=places,
                                                recent_actions=recent_actions, recent_obs=recent_obs,
                                                recent_scores=recent_scores, recent_reward=recent_reward,
                                                subgoals=modified_sg, curr_subgoal=curr_subgoal)
            ...
            if 'end subgoal' in predStrs or 'END_SG()' in predStrs:
                ...
                if 'No known action matches that input' in obs or 'Unknown action' in obs:
                    print('BBBBBBAAAAAADDDDD format')
                    action = findValidActionNew(predStrs, env, info['look'], recent_actions, sbert_model, logger, 5, False, taskName)
                    obs, reward, done, info = env.step(action)
            ...
            if not args['sg_']:
                ...
                if len(set(recent_scores[-50:]))== 1 and len(recent_scores)>50:
                    logger.info("Many recent actions do not change the score -- model is likely in a loop, stopping early.")
                    break
...
def eval_test(args, task_num, logger):
    ...
    for files in [file for file in os.listdir(f'{dir}') if file.endswith('.json')]:
        ...
        np.save(f'{args["output_path"]}/all_percentage_{args["seed"]}', np.array(all_percentage))
        np.save(f'{args["output_path"]}/all_scores_{args["seed"]}', np.array(all_scores))
    ...
    for files in [file for file in os.listdir(f'{dir}') if file.endswith('.json')]:
        ...
        for variation in variations:
            ...
            for i in range(len(indexes)):
                ...
                curr_subgoal = get_model_output(args, input_str_sg, tokenizer_sg, lm_model_sg, device, logger)[0]
                print('curr_subgoal is', curr_subgoal, 'curr target', targets[indexes[i]])
...
def parse_args():
    ...
    if not debug:
        ...
        parser.add_argument("--set", default="test", help="which dataset is going to be used, train or test")
        ...
        parser.add_argument("--compose_mode", default="v4_with_sg", help="prompt type for executer")
        parser.add_argument("--compose_mode_sg", type=str, default="v4_for_sg", help="prompt type for controller")
        ...
        parser.add_argument("--mode_sg", default="rand", help="the mode of generating sg; it can be either 'lm', 'rand', 'semi-rand'")
        parser.add_argument("--rand_percentage", type=int, default=1, help="it is for semi-random, or random, the persentage of chossing a wrong subgoal")
        parser.add_argument("--sg_", action='store_true', help="it can be either false or true. False means no sg in the the prompt")
        parser.add_argument("--first_sg_rand", action='store_true', help="it can be either false or true. False means no random selection for first sg")
        
        parser.add_argument("--test_mode", type=str, default="interactive", help=" 'sg' means using sg but not intractive, 'interactive', 'em' --> exact matching")
    else:
        parser.add_argument("--jar_path", type=str)
