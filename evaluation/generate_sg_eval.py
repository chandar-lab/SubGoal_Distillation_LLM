...
def sg_generator_eval(
    taskName, task_num, variation, task_desc, info_look, info_inventory
):
    ...
    if os.path.exists(sg_file_name):
        ...
        if df.loc[df.variations == variation, "chatgpt_output"].any():
            chatgpt_answer = df.loc[
                df.variations == variation, "chatgpt_output"
            ].values[0]
        else:
            ...
            recent_taskdecompition = pd.DataFrame(
                {"variations": [variation], "chatgpt_output": [chatgpt_answer]}
            )
            recent_taskdecompition.to_csv(
                sg_file_name, mode="a", index=True, header=False
            )
...
def create_initialprompt_prompt(sample1):
    ...
    messages.append({"role": "assistant", "content": sample1["subgoal"]})
