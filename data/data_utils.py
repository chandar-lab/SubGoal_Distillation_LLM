...
def compose_instance_v4_sg(
    ...
    if len(subgoals) > 1:
        input_str += ". Completed subtasks are: " + ", ".join(
            sg for sg in subgoals[:-1]
        )
    else:
...
def compose_instance_v4_sgaslabel(
    step_id,
    task_desc,
    returns_to_go,
    curr_action,
    ...
    input_str = (
        task_desc
        + f" </s> Time: {step_id}; Score: {int(recent_scores[-1] * 100)}; </s> "
    )

    if len(subgoals) > 2:
        input_str += ". Completed subtasks are: " + ", ".join(
            sg for sg in subgoals[:-2]
        )
    else:
        ### the completed subgoals are empty
        input_str += ". No completed subtasks"
...
def compose_instance_v4_sgaslabel_test(
    step_id,
    task_desc,
    returns_to_go,
    curr_action,
...
def compose_instance_v4_sgaslabel_v4(
    step_id,
    task_desc,
    returns_to_go,
    curr_action,
...
def compose_instance_v4_sg_nohist(
    step_id,
    task_desc,
    returns_to_go,
    curr_action,
    ...
    ind = 10
    for obs, action, reward in zip(
        recent_obs[-1:], recent_actions[-1:], recent_reward[-1:]
    ):
        input_str += f" <extra_id_{ind}> {formalize_action(action)} (+{int(reward * 100)}) --> {obs} | "
        ind -= 1
    input_str += " </s> "
...
def compose_instance_v4_nohist(
    step_id,
    task_desc,
    returns_to_go,
    curr_action,
...
def compose_instance_v4_nohist_2(
    mode,
    step_id,
    task_desc,
    returns_to_go,
...
def compose_instance_v4_first_sgaslabel(
    mode, step_id, task_desc, inventory, look, places, recent_scores, curr_subgoal
):
