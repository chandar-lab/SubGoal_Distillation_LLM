...
for mode in modes:
    ...
    data = []
    i = 0
    for file_name in [file for file in os.listdir(f"{dir}/data_dir/{mode}/") if file.endswith('.json')]:
        i += 1
        print(file_name, '----', i)
...
data = []
i = 0
for file_name in [file for file in os.listdir(f"{dir}/data_dir/{mode}/") if file.endswith('.val.json')]:
    i += 1
    print(file_name, '----', i)
    with open(f"{dir}/data_dir/{mode}/" + file_name) as json_file:
        for line in json_file:
